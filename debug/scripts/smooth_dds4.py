...
def smooth_gains(Yreal, Yimag, filter_size=5, deg=1):
    ...
    flag = res_real > np.percentile(res_real, 95, axis=0, keepdims=True)
    _Yreal[flag] = real[flag]
    res_imag = np.abs(Yimag - imag)
    flag = res_imag > np.percentile(res_imag, 95, axis=0, keepdims=True)
...
def main(data_dir, working_dir, obs_num):
    ...
    Yreal_full, Yimag_full = smooth_gains(Yreal_full, Yimag_full, 5, 2)
    # Npol, Nd, Na, Nf, Nt
    ...
    worst_dirs = np.argsort(np.square(diff_phase).mean(0).mean(0).mean(-1).mean(-1))[-4:]
    worst_ants = np.argsort(np.square(diff_phase).mean(0).mean(-1).mean(-1).mean(-1))[-4:]
    worst_times = np.argsort(np.square(diff_phase).mean(0).mean(0).mean(0).mean(0))[-4:]

    for d in worst_dirs:
        for a in worst_ants:
            plt.imshow(diff_phase[0,d,a,:,:], aspect='auto')
            plt.savefig(os.path.join(working_dir, 'phase_diff_dir{:02d}_ant{:02d}.png'.format(d,a)))
            plt.close('all')
            plt.imshow(diff_amp[0, d, a, :, :], aspect='auto')
