...
class TomographicKernel(Kernel):
    ...
    def __call__(self, X1, X2, bottom, width, l, sigma, *fed_kernel_params):
        x1 = X1[:, 0:3]  # N,3
        k1 = X1[:, 3:6]
        x2 = X2[:, 0:3]  # M,3
        k2 = X2[:, 3:6]
        ...
        K = lambda x, y: self.fed_kernel(x[None], y[None], l, sigma, *fed_kernel_params)[0,0] #* self.fed_kernel(x[None]-h, y[None]-h, width, 1., *fed_kernel_params)[0,0]
        Ky = lambda x, x2, k2: vmap(lambda x2, k2: ray_integral(lambda y: K(x, y), x2, k2))(*jnp.broadcast_arrays(x2, k2))
        Kxy = lambda x1, k1, x2, k2: vmap(lambda x1, k1: ray_integral(lambda x: Ky(x, x2, k2), x1, k1))(*jnp.broadcast_arrays(x1,k1))
