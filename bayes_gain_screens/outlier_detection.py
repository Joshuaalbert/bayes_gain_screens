...
class Classifier(object):
    ...
    def build_model(self, inputs, position_encoding, senders, receivers, output_bias=0.):
        with tf.variable_scope('classifier', reuse=tf.AUTO_REUSE):
            ...
            print(graph)
            sa1 = SelfAttention()
            gi1 = GraphIndependent(node_model_fn=lambda:snt.Sequential([snt.Linear(self.n_features), tf.nn.relu, snt.LayerNorm()]))
            graph = sa1(graph.nodes, graph.nodes, graph.nodes, graph)
            graph = gi1(graph)
            sa2 = SelfAttention()
            gi2 = GraphIndependent(node_model_fn=lambda:snt.Sequential([snt.Linear(1, use_bias=True), snt.LayerNorm()]))
